...
const NoteView: React.FC<NoteViewProps> = ({
  ...
  return <Card className="border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden h-full">
          ...
          <div className="flex gap-2">
            ...
            <Dialog>
              <DialogTrigger asChild>
                <Button variant="outline" size="sm" className="flex gap-1 rounded-full border-indigo-200 hover:bg-indigo-50 dark:border-indigo-900 dark:hover:bg-indigo-950" onClick={() => setShowConnections(true)}>
              ...
              <DialogContent className="max-w-3xl">
                ...
                <ScrollArea className="max-h-[70vh]">
                  <RelatedNotes note={note} backlinks={backlinks} suggestions={suggestedConnections} />
                </ScrollArea>
              </DialogContent>
            </Dialog>
      ...
      <CardFooter className="flex flex-wrap gap-1 border-t bg-slate-50/50 dark:bg-slate-900/50 py-3">
        {note.tags.map(tag => <Badge key={tag.id} style={{
